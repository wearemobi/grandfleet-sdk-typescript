...
[![npm version](https://img.shields.io/npm/v/@wearemobi/grandfleet-sdk.svg)](https://www.npmjs.com/package/@wearemobi/grandfleet-sdk)
...
The `@wearemobi/grandfleet-sdk` package provides type-safe TypeScript client libraries and utilities for interacting with **Grandfleet** sovereign cloud services, POS systems, inventory management, and double-entry accounting ledgers.
...
```bash
# Using pnpm
pnpm add @wearemobi/grandfleet-sdk

# Using npm
npm install @wearemobi/grandfleet-sdk

# Using yarn
yarn add @wearemobi/grandfleet-sdk
```
...
```typescript
import { GrandfleetClient, AuthModule, PosModule, FixedMoney } from '@wearemobi/grandfleet-sdk';
...
const loginResult = await auth.login('operator_walter', 'secret_hash_value');
console.log('Logged in successfully:', loginResult?.token);

// 3. POS Cart Calculation using Fixed-Scale Money ($10^-4)
const pos = new PosModule(client);
const cartResult = await pos.calculateCart([
  {
    sku: 'PROD-COFFEE-001',
    quantity: 2,
...
```typescript
import { FixedMoney } from '@wearemobi/grandfleet-sdk';
...
```typescript
import { ProblemDetailsError } from '@wearemobi/grandfleet-sdk';
